...
`src/pages/PlanPage.tsx` owns the state; `src/features/journey/` holds the parts. The search is **not** in the URL: it lived there so a journey could be shared and reached with the back button, and the cost was a query string of coordinates and labels on screen for the whole session. That was traded away deliberately — there is no shareable link to a search, and no bookmark that reopens one. The page's own memory of what it last searched is what stops a repeat; see `lastSearched`. There is no submit button — a complete form searches itself, and `JourneyForm` refuses to repeat a search whose inputs have not changed.
