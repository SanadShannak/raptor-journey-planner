...
- **Text aligns to a neighbour's glyphs, not its border.** A code or zone badge sits in a bordered chip, so its text starts a border and a padding in from the chip's own edge. A plain line above it with no padding is not aligned with it, it is aligned with the chip's box — which reads as the badge being indented. Match the chip's geometry (`border border-transparent px-1.5`) rather than eyeballing a pixel value.
