...
export default function ItineraryCard({ itineraryData, isSelected, ...props }) {
  {
    ...
    return (
      <div className="relative flex w-full h-full justify-center hover:bg-gray-200/40  transition-all duration-200 ease-out">
        {isSelected ? (
          <span className="absolute top-1/2 -translate-y-1/2 left-0 border-l-6 h-7/10   border-l-sky-600 "></span>
        ) : null}
        <div
          className={`flex flex-col gap-2 justify-around w-8/10 h-full py-3 border-b-2 border-gray-400/50`}
        >
          <div className="flex justify-between">
            <div className="text-lg md:text-base font-semibold text-gray-700">
              {itineraryData.startTime} - {itineraryData.endTime}
            </div>
            <div className="text-lg md:text-base font-semibold text-gray-700">
              ...
              if (leg.waitDurationMinutes > 0) {
                const waitPercentage =
                  (leg.waitDurationMinutes /
                    itineraryData.totalDurationMinutes) *
                  100;
